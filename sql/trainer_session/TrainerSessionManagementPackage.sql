...
CREATE OR REPLACE PACKAGE BODY PKG_TRAINER_SESSION_MGMT AS
    ...
    BEGIN
        ...
        IF v_conflict_count > 0 THEN
            RAISE_APPLICATION_ERROR(-20501, 'Trener o ID ' || p_trainer_id || ' ma już zaplanowaną sesję w tym przedziale czasowym.');
        END IF;
    ...
    BEGIN
        v_full_start_time := TO_TIMESTAMP(TO_CHAR(p_session_date, 'YYYY-MM-DD') || TO_CHAR(p_start_time, ' HH24:MI:SS'), 'YYYY-MM-DD HH24:MI:SS');
        v_full_end_time   := TO_TIMESTAMP(TO_CHAR(p_session_date, 'YYYY-MM-DD') || TO_CHAR(p_end_time,   ' HH24:MI:SS'), 'YYYY-MM-DD HH24:MI:SS');
        ...
        VALUES (trainer_sessions_seq.NEXTVAL, p_trainer_id, TRUNC(v_full_start_time), v_full_start_time, v_full_end_time)
    ...
    BEGIN
        ...
        SELECT ts.schedule_id, ts.trainer_id, tr.name, tr.surname, ts.session_date, ts.start_time, ts.end_time
            BULK COLLECT INTO v_sessions_tbl
        FROM trainer_sessions ts
                 JOIN trainers tr ON ts.trainer_id = tr.trainer_id
        WHERE ts.trainer_id = p_trainer_id
