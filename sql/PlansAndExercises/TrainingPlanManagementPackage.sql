CREATE OR REPLACE PACKAGE PKG_TRAINING_PLAN_MGMT AS
    ...
    TYPE ty_training_plan_record IS RECORD (plan_id training_plans.plan_id%TYPE, name training_plans.name%TYPE);
...
CREATE OR REPLACE PACKAGE BODY PKG_TRAINING_PLAN_MGMT AS
    ...
    BEGIN
        IF p_check_plan_exists THEN
            ...
            IF v_count = 0 THEN RAISE_APPLICATION_ERROR(-20300, 'Plan treningowy o ID ' || p_plan_id || ' nie istnieje.'); END IF;
        END IF;
        IF p_check_exercise_exists THEN
            SELECT COUNT(*) INTO v_count FROM exercises WHERE exercise_id = p_exercise_id;
            IF v_count = 0 THEN RAISE_APPLICATION_ERROR(-20011, 'Ćwiczenie o ID ' || p_exercise_id || ' nie istnieje.'); END IF;
        END IF;
        RETURN TRUE;
    ...
    BEGIN
        IF TRIM(p_name) IS NULL THEN RAISE_APPLICATION_ERROR(-20301, 'Nazwa planu treningowego nie może być pusta.'); END IF;
        INSERT INTO training_plans (plan_id, name) VALUES (training_plans_seq.NEXTVAL, TRIM(p_name)) RETURNING plan_id INTO p_plan_id;
        COMMIT;
    EXCEPTION WHEN OTHERS THEN ROLLBACK; p_plan_id := NULL; RAISE;
    ...
    BEGIN
        ...
        IF SQL%NOTFOUND THEN RAISE_APPLICATION_ERROR(-20300, 'Plan treningowy o ID ' || p_plan_id || ' nie istnieje.'); END IF;
        COMMIT;
    EXCEPTION WHEN OTHERS THEN ROLLBACK; RAISE;
    ...
    BEGIN
        ...
        RETURN v_cursor;
    EXCEPTION WHEN OTHERS THEN IF v_cursor%ISOPEN THEN CLOSE v_cursor; END IF; RAISE;
    ...
    PROCEDURE AddExerciseToPlan (p_plan_id IN training_exercise.plan_id%TYPE, p_exercise_id IN training_exercise.exercise_id%TYPE) AS
        v_is_valid BOOLEAN;
    BEGIN
        ...
        COMMIT;
    EXCEPTION WHEN DUP_VAL_ON_INDEX THEN ROLLBACK; RAISE_APPLICATION_ERROR(-20305, 'Ćwiczenie ID ' || p_exercise_id || ' jest już przypisane do planu ID ' || p_plan_id || '.');
    WHEN OTHERS THEN ROLLBACK; RAISE;
    END AddExerciseToPlan;

    PROCEDURE RemoveExerciseFromPlan (p_plan_id IN training_exercise.plan_id%TYPE, p_exercise_id IN training_exercise.exercise_id%TYPE) AS
        v_is_valid BOOLEAN;
    BEGIN
        v_is_valid := fn_validate_plan_exercise(p_plan_id, p_exercise_id);
        DELETE FROM training_exercise WHERE plan_id = p_plan_id AND exercise_id = p_exercise_id;
        IF SQL%NOTFOUND THEN RAISE_APPLICATION_ERROR(-20302, 'Nie znaleziono ćwiczenia ID '|| p_exercise_id ||' w planie ID '|| p_plan_id ||' do usunięcia.'); END IF;
        COMMIT;
    EXCEPTION WHEN OTHERS THEN ROLLBACK; RAISE;
    ...
    BEGIN
        ...
        OPEN v_cursor FOR SELECT e.exercise_id, e.name, e.description, e.group_id, mg.group_name AS MUSCLE_GROUP_NAME
                          ...
                          WHERE te.plan_id = p_plan_id ORDER BY e.name;
        RETURN v_cursor;
    EXCEPTION WHEN OTHERS THEN IF v_cursor%ISOPEN THEN CLOSE v_cursor; END IF; RAISE;
    ...
    BEGIN
        ...
        SELECT tp.plan_id, tp.name BULK COLLECT INTO v_plans_tbl
        FROM training_exercise te
                 JOIN training_plans tp ON te.plan_id = tp.plan_id
        WHERE te.exercise_id = p_exercise_id ORDER BY tp.name;
        RETURN v_plans_tbl;
    EXCEPTION WHEN OTHERS THEN RAISE;
    END GetPlansForExercise;

    FUNCTION GetTrainingPlanById (p_plan_id IN training_plans.plan_id%TYPE) RETURN SYS_REFCURSOR IS BEGIN RETURN GetTrainingPlanById_Func_Internal(p_plan_id); END;
    FUNCTION ListAllTrainingPlans RETURN SYS_REFCURSOR IS BEGIN RETURN ListAllTrainingPlans_Func_Internal(); END;
    FUNCTION GetExercisesForPlan (p_plan_id IN training_exercise.plan_id%TYPE) RETURN SYS_REFCURSOR IS BEGIN RETURN GetExercisesForPlan_Func_Internal(p_plan_id); END;
