CREATE OR REPLACE PROCEDURE prc_initialize_database AS
    ...
    FUNCTION fn_add_sequence(
        p_sql IN VARCHAR2,
        p_sequence_name VARCHAR2
    ) RETURN BOOLEAN IS
        temp NUMBER;
    BEGIN
        DBMS_OUTPUT.PUT_LINE('FN_ADD_SEQUENCE: Rozpoczęto dla sekwencji: ' || p_sequence_name);
        BEGIN
            SELECT COUNT(*)
            INTO temp
            FROM USER_SEQUENCES
        ...
        IF temp = 0 THEN
            DBMS_OUTPUT.PUT_LINE('FN_ADD_SEQUENCE: Próba utworzenia sekwencji ' || p_sequence_name);
            BEGIN
                EXECUTE IMMEDIATE p_sql;
                DBMS_OUTPUT.PUT_LINE('FN_ADD_SEQUENCE: Sekwencja ' || p_sequence_name || ' utworzona pomyślnie.');
                RETURN TRUE;
            EXCEPTION
                WHEN OTHERS THEN
            ...
            END;
        ELSE
            DBMS_OUTPUT.PUT_LINE('FN_ADD_SEQUENCE: Sekwencja ' || p_sequence_name || ' już istnieje.');
            RETURN TRUE;
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
